...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	if(session.getAttribute("username") != null) {	
		...
		if(Arrays.asList(roles).stream().anyMatch(r -> r.getRole().equals(Ruoli.ADMIN))) {
			...
			if(taskId == null) response.sendRedirect("progetti.jsp?error=task_not_found");
...
<div class="container">
			...
			<div class="table-responsive my-2" >
            	...
            	<table class="table table-striped" id="tabella">
                		...
                		<tbody>
                			<%
                			Object[] timesheets =Arrays.asList(AdminFacade.getInstance().getAllTimesheet())
                					.stream().filter( t-> t.getIdTask() == id).toArray();
                			for( int i = 0; i< timesheets.length; i++){
                				if(timesheets[i] instanceof Timesheet) {
