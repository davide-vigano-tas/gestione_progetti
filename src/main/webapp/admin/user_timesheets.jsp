...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	if(session.getAttribute("username") != null) {	
		...
		if(Arrays.asList(roles).stream().anyMatch(r -> r.getRole().equals(Ruoli.ADMIN))) {
			...
			if(id == null) response.sendRedirect("users.jsp?error=not_found");
...
<div class="container">
			...
			<h3>Timesheet del dipendente <%= u_id %></h3>
		</header>
			...
			<div class="table-responsive my-2" >
            	...
            	<table class="table table-striped" id="tabella">
                		...
                		<tbody>
                			<%
                			List<Timesheet> timesheets = AdminFacade.getInstance().getTimesheetsByDipendente(u_id);
                			for( int i = 0; i< timesheets.size(); i++){
                				
                					Timesheet obj = timesheets.get(i);
                					ProjectTask task = AdminFacade.getInstance().getTaskById(obj.getIdTask());
                			%>
                			<tr>
                				<td><%=obj.getId() %></td>
                				<td>
			       			<a type="submit" class="btn btn-default btn-sm" 
												href="/<%= application.getServletContextName()%>/admin/dettagliProgetto.jsp?id=<%=obj.getIdProgetto()%>">
														<%=AdminFacade.getInstance().getProjectById(obj.getIdProgetto()).getNomeProgetto() %>
												</a>
								</td>
                				<td><%=obj.getOreLavorate() %></td>
                				<td><%=task.getStato().name() %></td>
                				<td><%=obj.getData()%></td>
                						<td>	<%
                					String stato;
